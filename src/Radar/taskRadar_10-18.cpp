...
CSocketSender* psender = new CSocketSender("192.168.31.178", 15648);
...
//            fwrite(&pFrame->bincnt, 4, 1, fid);
    ...
    int buffer_write(int antNum, int frameno, float* data, int cnt)
    {
       printf("antNum :%d frameno :%d,cnt:%d\n",antNum, frameno,cnt);
        if(cnt<=0)
        {
            return 0;
        }
...
//                    }
        ...
        //m_x4_buf->x4data[m_x4_buf->write_ptr].head = head;
        m_x4_buf->x4data[m_x4_buf->write_ptr].antNum = antNum;
...
//        printf("read_sz:%d\n",read_sz);
        ...
        {
            fwrite(&m_x4_buf->x4data[m_x4_buf->read_ptr].head, 16 ,1, fid);
            fwrite(&m_x4_buf->x4data[m_x4_buf->read_ptr].antNum, 4, 1, fid);
...
float32_t data_frame_normolized[138];
...
*/
    ...
    //sleep(1);
    status = x4driver_read_frame_normalized(x4driver,&frame_counter,data_frame_normolized, bin_count);

    if(XEP_ERROR_X4DRIVER_OK == status)
    {
        g_readDone = 1;
       // printf("frame :%d.\n", frame_counter);
       // g_frameCache.AddData(1, frame_counter,data_frame_normolized, fdata_count);
        if(g_antNum == 5)
        {
           // printf("g_antNum 5:%d.\n", g_antNum);
            g_frameCache.AddData(1, frame_counter,data_frame_normolized, fdata_count);
           // g_frameCache.buffer_write(1, frame_counter,data_frame_normolized, fdata_count);

        }
        else if(g_antNum == 4)
        {
               // memset(data_frame_normolized,0, sizeof(data_frame_normolized));
               g_frameCache.AddData(2, frame_counter,data_frame_normolized, fdata_count);
              // g_frameCache.buffer_write(2,frame_counter,data_frame_normolized, fdata_count);
...
int taskRadar(void)
{
    ...
    status = x4driver_set_sweep_trigger_control(x4driver, SWEEP_TRIGGER_MANUAL); // By default let sweep trigger control done by X4SWEEP_TRIGGER_MANUAL
#ifdef DEBUG
    if(status == XEP_ERROR_X4DRIVER_OK)
    {
