...
typedef struct _X4FRAMEDATA
{
    ...
    uint32_t frameno;
    uint32_t bincnt;
    uint32_t fall;//1=fall,0=no fall
    float* bins;
...
int taskRadar(void)
{
    ...
    status = x4driver_set_sweep_trigger_control(x4driver, xgl_sweep_trigger_control); // MANUAL or let sweep trigger control done by X4
#ifdef DEBUG
    if(status == XEP_ERROR_X4DRIVER_OK)
    {
...
/***************set fps, this will trigger data output***************/
    ...
    status=x4driver_set_fps(x4driver, xgl_fps); // Generate 1000 frames per second
...
/////////////////////////////tianyue////////////////////////////////////
    // status = x4driver_disable_random (x4driver);
    // //status = x4driver_set_tx_power(x4driver, TX_POWER_OFF);
    // if (status != XEP_ERROR_X4DRIVER_OK)
    // {
    //     printf("Random Disable failed");
    //     return 1;
