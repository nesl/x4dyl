...
uint32_t _update_normalization_variables(X4Driver_t* x4driver)
{
    ...
    xtx4_tx_center_frequency_t center = TX_CENTER_FREQUENCY_KCC_8_748GHz;
...
int x4driver_create(X4Driver_t** x4driver, void* instance_memory, X4DriverCallbacks_t* x4driver_callbacks,X4DriverLock_t *lock,X4DriverTimer_t *timer,X4DriverTimer_t *timer_action, void* user_reference)
{
    ...
    d->center_frequency = TX_CENTER_FREQUENCY_KCC_8_748GHz;
 ...
 * @return Status of execution as defined in x4driver.h.
 */
extern int g_readDone;

char spi_buffer[2500];
int x4driver_read_frame_normalized(X4Driver_t* x4driver, uint32_t* frame_counter, float32_t* data, uint32_t length)
{
    ...
    status = x4driver_read_frame_bytes(x4driver, frame_counter,spi_buffer, x4driver->frame_read_size);
   //  = 1;
    if (x4driver->downconversion_enabled == 0)
    {
        _x4driver_unpack_and_normalize_frame(x4driver,data,length,spi_buffer,x4driver->frame_read_size);
    }
    else
    {
        _x4driver_unpack_and_normalize_downconverted_frame(x4driver,data,length,spi_buffer,x4driver->frame_read_size);
    }
...
static uint32_t bit_is_set(uint32_t data,uint32_t bitnr)
{
    ...
    //printf("data ：%d ; bitnr %d  (1<< bitnr) %d res  %d\n", data, bitnr, (1<< bitnr), res);
    uint32_t ret = res != 0;
   // printf("return %d", ret);
    return ret;
}
...
int _x4driver_set_internal_register(X4Driver_t* x4driver, uint8_t address, uint8_t write_value, uint8_t command)
{
    ...
    //printf("ADDR_SPI_TO_CPU_WRITE_DATA_WE  %d address %d\n",ADDR_SPI_TO_CPU_WRITE_DATA_WE | 0x80, address);
...
int _x4driver_get_framecounter(X4Driver_t* x4driver, uint32_t * value)
{
    ...
    while (bit_is_set(fifo_status,FROM_CPU_VALID_BIT) == BIT_SET)
    {
        ...
        //printf("x4driver_get_spi_register %d \n",tmp_rb );
        x4driver_get_spi_register(x4driver,ADDR_SPI_SPI_MB_FIFO_STATUS_R,&fifo_status);
        data_in_fifo++;
    }
...
int x4driver_init(X4Driver_t* x4driver)
{
    ...
    x4driver->center_frequency = TX_CENTER_FREQUENCY_KCC_8_748GHz;
   ...
   //     x4driver->initialized = 0;
    ...
    //    return status;
    //}
   // status = x4driver_set_tx_center_frequency(x4driver, 4);
    /////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////tianyue///////////////////////////////
int x4driver_disable_random (X4Driver_t* x4driver)
{
    uint32_t status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
    status = x4driver_set_pif_register(x4driver, ADDR_PIF_TRX_LFSR_RESET_WS ,0xff);
    //status = x4driver_set_pif_segment(x4driver, ADDR_PIF_TRX_CTRL_MODE_RW,0x04,0x01);
    
    //status = x4driver_set_pif_segment(x4driver, ADDR_PIF_TRX_LFSR_TAPS_0_RW,0xFF,0x00);
    //status = x4driver_set_pif_segment(x4driver, ADDR_PIF_TRX_LFSR_TAPS_1_RW,0xFF,0x00);
    //status = x4driver_set_pif_segment(x4driver, ADDR_PIF_TRX_LFSR_TAPS_2_RW,0xFF,0x00);
